...
class SHActivityView: UIView {
    ...
    func showAndStartAnimate()
    {
        ...
        else if (spinnerSize == .kSHSpinnerSizeMedium || spinnerSize == .kSHSpinnerSizeLarge || spinnerSize == .kSHSpinnerSizeVeryLarge)
        {
            ...
            if (centerTitle != nil)
            {
                labelCenter = UILabel(frame:CGRect(x:(viewNotRotate?.frame.size.width)!/4,y: ((viewNotRotate?.frame.size.height)!/2)-15,width:(viewNotRotate?.frame.size.width)!-(2*((viewNotRotate?.frame.size.width)!/4)),height:30))
            ...
            if (bottomTitle != nil)
            {
                ...
                if(stopBottomTitleDotAnimation == nil || stopBottomTitleDotAnimation == false)
                {
                    let labelDot = UILabel(frame:CGRect(x:labelBottom!.frame.origin.x + labelBottom!.frame.size.width + 1, y:labelBottom!.frame.origin.y, width:CGFloat(widthLabelDot),height:labelBottom!.frame.size.height))
                    if (bottomTitleFont != nil)
                    {
